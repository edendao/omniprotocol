...
contract LayerZeroEndpointMock is ILayerZeroEndpoint {
  ...
  function send(
    ...
    address, // layerZeroPaymentAddress,
    ...
    // Mock the relayer paying the dstNativeAddr the amount of extra native token
    {
      ...
      assembly {
        dstNative := mload(add(adapterParams, 66))
        dstNativeAddr := mload(add(adapterParams, 86))
      }
    }
  ...
  function receiveAndForward(
    ...
    ILayerZeroReceiver(_destAddr).lzReceive(
      _srcChainId,
      _srcAddress,
      _nonce,
      _payload
  ...
  function hasStoredPayload(uint16, bytes memory)
    external
    pure
    override
    returns (bool)
