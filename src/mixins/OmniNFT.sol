...
/*
 * An OmniNFT is an app-specific cross-chain identity for the omnichain future.
 */
contract OmniNFT is Authenticated, Omnichain, IERC721, IERC721Metadata {
  event Sync(
  ...
  mapping(uint256 => OmniToken) public omniToken;
  ...
  function setToken(
    ...
    omniToken[id].uri = uri;
    omniToken[id].metadata = metadata;
  }
  ...
  function lzReceive(
    ...
    (address owner, OmniToken memory data) = abi.decode(
      payload,
      (address, OmniToken)
    );
